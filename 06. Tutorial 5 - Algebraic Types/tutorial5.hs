...
getOrangeSegmetns :: Fruit -> Int
getOrangeSegmetns (Apple(_)) = 0
getOrangeSegmetns (Orange(variety, segments)) = segments

getAppleWorms :: Fruit -> Bool
getAppleWorms (Orange(_)) = False
getAppleWorms (Apple(_, hasWorm)) = hasWorm

-- 1.
isBloodOrange :: Fruit -> Bool
isBloodOrange (Apple(_)) = False
isBloodOrange (Orange(variety, _))
						| (variety == "Moro") || (variety == "Tarocco") || (variety == "Sanguinello") = True
						| otherwise = False

-- 2. -- how to access the second member of the Fruit Data Type
bloodOrangeSegments :: [Fruit] -> [Fruit]
bloodOrangeSegments fruits = [x | x <- fruits, isBloodOrange x]

-- 3.
worms :: [Fruit] -> Int
worms fruits = length [x | x <- fruits, getAppleWorms x]
